...
func newCsiChartInstallAction(p *payloadOpts, csiConfig *CsiConfig) *types.ChartInstallAction {
	...
	chartInstall := newChartInstall(p.Name, p.InstallOptions.Version, p.InstallOptions.ClusterID, p.InstallOptions.ClusterName, p.Host, rancherChartsName, p.ProjectID, p.DefaultRegistry, csiValues)
